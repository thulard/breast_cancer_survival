...
Now we want to analyse the survival time of the patient depending on whether or not they have a chemotherapy.
...
attr(res.sum, "table")
```
We can observe it seems there is a difference between the 2 groups. Indeed, people without any chemotherapy tend to survive longer:  50 % of survivors after 300 months against 179 months for people with chemotherapy.
...
## Kaplan Meier analysis for all the binary variables
...
Looking at all binary variables, we can observe that only the "type_of_breast_surgery" and the "chemotherapy" have a significant impact on the survival.
...
The overall goal is to provide recommendation on the most efficient treatment (higher survival in months) for a designed group of person. The design group of person will be done through cox analysis with the use of the features:
...
It will make more sense to choose Stage 1 as a reference.
...
_   for Stage2, Stage3 and Stage4 $\beta$ is positive so there a higher risk for these groups compare to Stage1 group
...
By best treatment, we mean a treatment that provides the longer survival in months.
...
```{r}
cox_group_4 <- coxph(Surv(duration, status) ~ type_of_breast_surgery + chemotherapy +
                       hormone_therapy + radio_therapy, data = df_subset[df_subset$tumor_stage==4,])
```
...
# With new model for cox group 2 (TO BE REMOVED)

```{r}
cox_group_2_bis <-  coxph(Surv(duration, status) ~ type_of_breast_surgery + hormone_therapy , data = df_subset[df_subset$tumor_stage==2,])
```
...
```{r}
test.ph_bis <- cox.zph(cox_group_2_bis)
ggcoxzph(test.ph_bis)
```

## Schoenfeld test on all models
```{r}
test.ph_1 <- cox.zph(cox_group_1)
test.ph_2 <- cox.zph(cox_group_2)
test.ph_3 <- cox.zph(cox_group_3)
test.ph_4 <- cox.zph(cox_group_4)
...
# Conclusion (WARNING TO BE UPDATE)
